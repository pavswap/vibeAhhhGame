...
def _gen_void_walls(count, r):
    """Return list of (orientation, position_frac) pairs.
    Walls are kept between 0.20 and 0.80 of screen size, and at least
    0.15 apart from each other so they don't cluster."""
    out=[]; used_fracs=[]
    # Build a pool of orientations: mix vertical and horizontal
    pool = (["vertical","horizontal"] * 4)[:count*2]
    r.shuffle(pool)
    for ori in pool:
        if len(out) >= count: break
        for _ in range(60):
            frac = r.uniform(0.22, 0.78)
            if all(abs(frac-f) > 0.15 for f in used_fracs):
...
def generate_level(level_idx, seed=None):
    ...
    void_walls = []
    if is_void:
        # L13→1 wall, scales to 3 by L16+, capped at 3
        count = min(3, 1 + (level_idx - TIER_SPINNER_END) // 2)
        void_walls = _gen_void_walls(count, r)
