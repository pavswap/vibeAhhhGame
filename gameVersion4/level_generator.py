...
def _gen_void_walls(count, r):
    ...
    Always returns exactly 2 walls: one horizontal and one vertical.
    Walls are kept between 0.30 and 0.70 of screen size."""
    out = []
    
    # Always create one horizontal wall
    h_frac = r.uniform(0.30, 0.70)
    out.append(("horizontal", h_frac))
    
    # Always create one vertical wall
...
def generate_level(level_idx, seed=None):
    ...
    void_walls = []
    if is_void:
        # Always generate 2 walls: one horizontal, one vertical
        void_walls = _gen_void_walls(2, r)
